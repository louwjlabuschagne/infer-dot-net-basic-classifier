...
namespace Models
{
	...
	/// 
	/// Generated by Infer.NET 0.3.1912.403 at 04:38 on Wednesday, 27 January 2021.
	/// </remarks>
	public partial class Model_EP : IGeneratedAlgorithm
	{
		...
		private void Changed_featureValue_isSetosa_numberOfIterations(int numberOfIterations)
		{
			...
			this.threshold_F = GaussianFromMeanAndVarianceOp.SampleAverageConditional(-5.0, 10.0);
			...
			for(int flower = 0; flower<100; flower++) {
				weight_rep_F[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				score_F[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				noisyScore_F[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				vdouble11_F[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				vdouble11_B[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				threshold_rep_F[flower] = Gaussian.Uniform();
			}
			...
			for(int flower = 0; flower<100; flower++) {
				noisyScore_use_B[flower] = Gaussian.Uniform();
			}
			...
			for(int iteration = this.numberOfIterationsDone; iteration<numberOfIterations; iteration++) {
				...
				for(int flower = 0; flower<100; flower++) {
					// Message to 'threshold_rep' from Replicate factor
					threshold_rep_F[flower] = ReplicateOp_Divide.UsesAverageConditional<Gaussian>(this.threshold_rep_B[flower], this.threshold_rep_F_marginal, flower, threshold_rep_F[flower]);
					// Message to 'weight_rep' from Replicate factor
					weight_rep_F[flower] = ReplicateOp_Divide.UsesAverageConditional<Gaussian>(this.weight_rep_B[flower], this.weight_rep_F_marginal, flower, weight_rep_F[flower]);
					// Message to 'score' from Product factor
					score_F[flower] = GaussianProductOpBase.ProductAverageConditional(this.FeatureValue[flower], weight_rep_F[flower]);
					// Message to 'noisyScore' from GaussianFromMeanAndVariance factor
					noisyScore_F[flower] = GaussianFromMeanAndVarianceOp.SampleAverageConditional(score_F[flower], 10.0);
					// Message to 'vdouble11' from Difference factor
					vdouble11_F[flower] = DoublePlusOp.AAverageConditional(noisyScore_F[flower], threshold_rep_F[flower]);
					// Message to 'vdouble11' from IsPositive factor
					vdouble11_B[flower] = IsPositiveOp_Proper.XAverageConditional(Bernoulli.PointMass(this.IsSetosa[flower]), vdouble11_F[flower]);
					// Message to 'noisyScore_use' from Difference factor
					noisyScore_use_B[flower] = DoublePlusOp.SumAverageConditional(vdouble11_B[flower], threshold_rep_F[flower]);
					// Message to 'score' from GaussianFromMeanAndVariance factor
					score_B[flower] = GaussianFromMeanAndVarianceOp.MeanAverageConditional(noisyScore_use_B[flower], 10.0);
					// Message to 'weight_rep' from Product factor
					this.weight_rep_B[flower] = GaussianProductOpBase.BAverageConditional(score_B[flower], this.FeatureValue[flower]);
					// Message to 'threshold_rep' from Difference factor
					this.threshold_rep_B[flower] = DoublePlusOp.BAverageConditional(noisyScore_F[flower], vdouble11_B[flower]);
					this.threshold_rep_F_marginal = ReplicateOp_Divide.MarginalIncrement<Gaussian>(this.threshold_rep_F_marginal, threshold_rep_F[flower], this.threshold_rep_B[flower]);
					this.weight_rep_F_marginal = ReplicateOp_Divide.MarginalIncrement<Gaussian>(this.weight_rep_F_marginal, weight_rep_F[flower], this.weight_rep_B[flower]);
				}
				// Message to 'weight_rep' from Replicate factor
		...
		private void Changed_numberOfIterationsDecreased_Init_featureValue_isSetosa(bool initialise)
		{
			...
			for(int flower = 0; flower<100; flower++) {
				this.threshold_rep_B[flower] = Gaussian.Uniform();
				this.weight_rep_B[flower] = Gaussian.Uniform();
			}
